...
$HintPath = $FilePath + ".hint"

function Invoke-AesEncryption {
    param($Path, $Password, $ActionMode)
...
if ($Action -and $Password) {
    if ($Action -in @('Encrypt', 'Decrypt')) {
        $success = Invoke-AesEncryption -Path $FilePath -Password $Password -ActionMode $Action
    ...
    switch ($opcion) {
        '1' {
            if (-not (Test-Path $FilePath)) {
                ...
                $success = Invoke-AesEncryption -Path $FilePath -Password $pass -ActionMode "Encrypt"
                if ($success) {
        ...
        '2' {
            if (-not (Test-Path $EncPath)) {
                ...
                $null = Invoke-AesEncryption -Path $FilePath -Password $pass -ActionMode "Decrypt"
            }
        }
        '3' {
            ...
            $decrypted = Invoke-AesEncryption -Path $FilePath -Password $oldPass -ActionMode "Decrypt"
            ...
            if ($decrypted) {
                ...
                $encrypted = Invoke-AesEncryption -Path $FilePath -Password $newPass -ActionMode "Encrypt"
